...
const User = require("../../../server/models/uers");
...
describe("Users controller", () => {
  ...
  describe("sigin", () => {
    it("should return token when signIn called", () => {
      ...
      return userController.signIn(res, res).then(() => {
        expect(res.status).to.have.been.calledWith(200);
        expect(res.json.callCount).to.equal(1);
      });
    });

    it("shuld return fake token using rewire", () => {
      sandbox.spy(res, "json");
      sandbox.spy(res, "status");
  ...
  describe("signUp", () => {
    it("should retnr 403 if the user is already save in the db.", () => {
      sandbox.spy(res, "json");
      sandbox.spy(res, "status");
      sandbox
        .stub(User, "findOne")
    ...
    it("should return 200 if user is not in db and it was saved", () => {
      ...
      sandbox
        ...
        .return(Promise.resolve({ id: faker.random.number() }));

      // next test we will fake token with our fake token to see that
      // json body has been called eith it at the moment we only count
    ...
    it("shold return 200 if user is not in db using callback done", (done) => {
      sandbox.spy(res, "json");
      sandbox.spy(res, "status");
      sandbox.stub(User, "findOne").returns(Promise.resolve(false));
      sandbox
      ...
      // example with done callback, we will call it in the function
    ...
    it("shold return fake token in res.json", (done) => {
      sandbox.spy(res, "json");
      sandbox.spy(res, "status");
      sandbox.stub(User, "findOne").returns(Promise.resolve(false));
      sandbox
